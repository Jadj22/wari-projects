...
export default function EditResultPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const [resultResponse, gamesResponse] = await Promise.all([
                    axios.get(`${API_URL}admin/results/${id}/`, {
                ...
                setFormData({
                    ...
                    result_date: resultData.result_date
                        ? new Date(resultData.result_date).toISOString().slice(0, 16)
                        : "",
                    outcome: resultData.outcome || "",
                    outcome_details: resultData.outcome_details
                        ? JSON.stringify(resultData.outcome_details, null, 2)
                        : "",
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await axios.patch(`${API_URL}admin/results/${id}/`, payload, {
                headers: { Authorization: `Bearer ${accessToken}` },
            });
        ...
        return (
            <div className="flex items-center justify-center min-h-screen bg-gray-100">
                <div className="text-center">
                    ...
                    <p className="mt-2">Seuls les administrateurs et éditeurs peuvent modifier des résultats.</p>
                </div>
            </div>
        );
    }

    if (!result) return <p className="text-center text-lg">Résultat non trouvé.</p>;

    return (
        <div className="p-6 max-w-2xl mx-auto">
            <h1 className="text-2xl font-bold mb-6">Modifier le résultat : {result.game_name}</h1>
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                >
                    {loading ? "Mise à jour en cours..." : "Mettre à jour le résultat"}
                </button>
            </form>
        </div>
    );
